...
Static HTML/CSS/JS medical practice site (no build step) on GitHub Pages + Cloudflare. Pushes to `main` deploy instantly (workers deploy manually — see below).
...
- `docs/superpowers/specs/2026-08-08-unified-savianu-site-design.md` — architecture + redirect map (§10, **now implemented** in the Cloudflare workers, see below)
...
- Cloudflare workers are ESM but `cloudflare/package.json` is `"type": "commonjs"` → plain `node --check cloudflare/worker.js` fails on `export default`. Syntax-check with: `Get-Content -Raw cloudflare/worker.js | node --input-type=module --check` (same for `redirect-worker.js`).
...
- **Cache-busting `?v=N`**: one consistent version per file across ALL pages. Current: `styles.css?v=37`, `app.js?v=27` (grep to confirm if the file looks stale). Bump when changing that file. The new-page skill template still shows stale `?v=24`/`?v=18` — use the real current values.
- **sw.js `savianu-vN`**: the `bump-sw.mjs` PostToolUse hook (`.claude/settings.json`, matcher `Edit|Write`) is Claude-Code tooling and does NOT fire reliably in this OpenCode environment (verified: no bumps across repeated Edit/Write calls). Run `node tools/update-sw.mjs` yourself when the precache list or version needs a bump: it regenerates `PRECACHE_URLS` and bumps `savianu-vN` by +1. Run it once at the end of any session that added/removed pages.
- **sitemap.xml / robots.txt are hand-maintained** (no tool): add new public pages to `sitemap.xml` and update every `<lastmod>` to the current date (Google reads it — stale dates signal "not updated"); `colleghi/*` and `offline.html`/`404.html` are deliberately absent (noindex). robots.txt already disallows `colleghi/xsegretarie.html`, `colleghi/RUAP/`, `colleghi/gestoreturni/` — keep it that way.
...
- **Cloudflare workers — two separate deploys, both manual**: `cloudflare/worker.js` = savianu.it security headers + legacy-path 301s (deploy: `cd cloudflare && npx wrangler deploy`); `cloudflare/redirect-worker.js` = dottemanuelsavianu.it → savianu.it 301s (deploy: `cd cloudflare && npx wrangler deploy --config wrangler-redirect.toml`). Both carry the spec §10 redirect map — keep the maps in sync; the one intentional difference is `/malattia.html` → `/colleghi/malattia.html` (dott domain) vs `/ssn/malattia.html` (savianu.it). **Pushing to main does NOT deploy workers** — after ANY edit to a worker file you must run its deploy command. CSP `script-src` must keep ALL of: `calendar.google.com`, `translate.google.com`, `ssl.google-analytics.com` + the tool-page CDNs `cdn.tailwindcss.com`, `cdnjs.cloudflare.com`, `cdn.jsdelivr.net`, `unpkg.com`, `cdn.sheetjs.com` — self-contained pages (RUAP, gestoreturni, calcolatori, ssn/malattia) load Tailwind/chart.js/react/jsPDF/xlsx from these and break completely if removed. `frame-src https://calendar.google.com` + Google Translate domains too. Never re-add `cloudflare/node_modules`.
- **New pages**: follow `.claude/skills/new-page/SKILL.md` template (`<site-nav data-section="...">`/`<site-footer>` fallbacks, single module `app.js` tag); keep single `<h1>`, skip link, ferie-banner hook, `data-i18n` (patient pages). Every public page carries a self-referencing `<link rel="canonical" href="https://savianu.it/...">` (never ending in `/index.html` — section dashboards canonicalize to `/ssn/`, `/privati/`, `/colleghi/`). Link section dashboards in directory form (`ssn/`, `../privati/`), not `index.html`. After adding a major page, run `node tools/update-sw.mjs` to regenerate the precache list.
