...
- **Cloudflare worker** (`cloudflare/worker.js`): CSP allows `calendar.google.com` frames + Google Translate domains — keep them when editing headers. Deploy: `cd cloudflare && npx wrangler deploy`. Never re-add `cloudflare/node_modules`.
